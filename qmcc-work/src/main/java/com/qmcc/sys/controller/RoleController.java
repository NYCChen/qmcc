...
public class RoleController {
    ...
    public DataGridView initPermissionByRoleId(Integer roleId){
         ...
         */
        List<Integer> currentRolePremissions = this.roleService.queryRolePermissionIdsByRid(roleId);
        List<Permission> carrentPermissions = null;
        if (currentRolePremissions.size()>0){
            queryWrapper.in("id", currentRolePremissions);
